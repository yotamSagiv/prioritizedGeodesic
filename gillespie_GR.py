...
behav_temp = 0.35  # Softmax temperature on behavioural choice

num_replay_steps = 3
replay_lr = 0.05  # Goal distribution value learning rate

pre_replay_forgetting = True
decay_rate = 0.94  # GR decay rate on every step
...
for session in range(num_sessions):
	...
	replay_goal_vals = (1 / num_arms) * np.ones(num_arms)  # Will remain a distribution under certain assumptions
	...
	for trial in range(num_trials):
		...
		for adx, action in enumerate(act_seq):
			...
			ga.basic_learn((ga.curr_state, action, next_state), decay_rate=1, noise=noise)  # Update GR
			...
			if adx == len(act_seq) - 1:
				# Update the value functions for each arm as Rescorla-Wagner linear interpolation
				if reward == 0:
					target = (1 / (num_arms - 1)) * np.ones(num_arms)
					target[chosen_arm] = 0
				else:  # Not a distribution for reward != 1...
...
# Save everything
np.savez('./Data/gillespie/GR.npz', prec=prechoice_replays, posto=postoutcome_replays, state_trajs=states_visited,
